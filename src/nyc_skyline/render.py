...
view.ui.move("zoom","bottom-left");view.whenLayerView(buildings).then(layerView=>{{buildingLayerView=layerView;return reactiveUtils.whenOnce(()=>!layerView.updating);}}).then(hideLoading).catch(error=>{{hideLoading();console.error("Electric City scene loading failed",error);}});
let clickRequest=0;
