...
.search-panel{{position:fixed;z-index:5;top:132px;left:34px;width:286px;padding:12px;border:1px solid #273e58;border-radius:12px;background:#07111fea;box-shadow:0 12px 34px #0008;backdrop-filter:blur(12px)}}
...
.esri-ui-corner{{top:112px}}@media(max-width:700px){{.brand{{top:18px;left:18px}}.brand h1{{font-size:34px}}.search-panel{{top:112px;left:14px;right:14px;width:auto}}.controls{{top:auto;bottom:14px;left:14px;right:14px;overflow:auto}}.legend{{left:14px;right:14px;bottom:68px}}.legend b small{{display:block;margin:4px 0 0}}.legend-item{{margin-bottom:3px}}.inspector{{left:14px!important;right:14px;top:auto!important;bottom:150px;width:auto}}}}
...
<section class="search-panel" aria-label="Search named Manhattan buildings"><label for="buildingSearch">FIND A NAMED BUILDING</label><div class="search-row"><input id="buildingSearch" type="search" list="namedBuildings" placeholder="Loading building names…" autocomplete="off" disabled /><button id="searchGo" disabled>GO</button></div><datalist id="namedBuildings"></datalist><label class="filter-toggle"><input id="namedOnly" type="checkbox" disabled /> Show only named buildings</label><small id="searchStatus" aria-live="polite">Loading the 717 known names…</small></section>
...
const searchInput=document.getElementById("buildingSearch"),searchGo=document.getElementById("searchGo"),searchList=document.getElementById("namedBuildings"),namedOnly=document.getElementById("namedOnly"),searchStatus=document.getElementById("searchStatus");
...
async function loadNamedBuildings(){{const query=buildings.createQuery();query.where=NAMED_WHERE;query.outFields=["OBJECTID","NAME"];query.returnGeometry=false;query.orderByFields=["NAME ASC"];const result=await buildings.queryFeatures(query);namedBuildingRecords=result.features.map(feature=>({{id:feature.attributes.OBJECTID,name:String(feature.attributes.NAME||"").trim()}})).filter(record=>record.name).sort((a,b)=>a.name.localeCompare(b.name));const fragment=document.createDocumentFragment();namedBuildingRecords.forEach(record=>{{const option=document.createElement("option");option.value=record.name;fragment.append(option);}});searchList.replaceChildren(fragment);searchInput.disabled=false;searchGo.disabled=false;searchInput.placeholder="Empire State Building…";searchStatus.textContent=`${{namedBuildingRecords.length.toLocaleString()}} named buildings ready`;}}
...
view.ui.move("zoom","bottom-left");view.whenLayerView(buildings).then(layerView=>{{buildingLayerView=layerView;namedOnly.disabled=false;return reactiveUtils.whenOnce(()=>!layerView.updating);}}).then(hideLoading).catch(error=>{{hideLoading();console.error("Electric City scene loading failed",error);}});
...
searchGo.addEventListener("click",jumpToNamedBuilding);searchInput.addEventListener("keydown",event=>{{if(event.key==="Enter"){{event.preventDefault();jumpToNamedBuilding();}}}});searchInput.addEventListener("change",()=>{{if(findNamedBuilding(searchInput.value))jumpToNamedBuilding();}});
