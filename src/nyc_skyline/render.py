...
function endDriveImmediately(){{driveRun++;view.animation?.stop();if(introRoad.parent)map.remove(introRoad);view.camera=cameras.skyline;driveIntro.hidden=true;driveIntro.className="drive-intro";document.body.classList.remove("intro");}}
async function startDrive(){{const run=++driveRun;closeInspector();searchResults.hidden=true;document.body.classList.add("intro");driveIntro.hidden=false;driveIntro.className="drive-intro";driveStage.textContent="BQE NIGHT RUN";driveSpeedValue.textContent="88";if(!introRoad.parent)map.add(introRoad,0);view.camera=driveCameras[0];try{{await pause(450);if(run!==driveRun)return;driveStage.textContent="ATLANTIC AVE · QUEENS-BOUND";await view.goTo(driveCameras[1],{{duration:1900,easing:"linear"}});if(run!==driveRun)return;driveIntro.classList.add("slowmo");driveStage.textContent="CITY OF DREAMS";driveSpeedValue.textContent="42";await view.goTo(driveCameras[2],{{duration:3600,easing:"ease-in-out"}});if(run!==driveRun)return;driveStage.textContent="BROOKLYN HEIGHTS · SLOW MOTION";await view.goTo(driveCameras[3],{{duration:2100,easing:"ease-in-out"}});if(run!==driveRun)return;driveIntro.classList.add("to-map");await pause(700);if(run!==driveRun)return;if(introRoad.parent)map.remove(introRoad);view.camera=cameras.skyline;document.body.classList.remove("intro");driveIntro.classList.add("done");await pause(900);if(run===driveRun)driveIntro.hidden=true;}}catch(error){{if(error?.name!=="AbortError")console.error("Electric City BQE drive failed",error);endDriveImmediately();}}}}
